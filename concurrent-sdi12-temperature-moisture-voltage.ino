...
//-------------------------------------------------------------------humidity--------------------------------------
bool getResultsHumidity(char i, int resultsExpected) {
...
int startConcurrentMeasurementHumidity(char i, String meas_type = "") {
  mySDI12.clearBuffer();
  String command = "";
  command += i;
  command += "C";
...
bool getResultsTemperature(char i, int resultsExpected) {
  uint8_t resultsReceived = 0;
  uint8_t cmd_number      = 0;
  while (resultsReceived < resultsExpected && cmd_number <= 9) {
    String command = "";
...
int startConcurrentMeasurementTemperature(char i, String meas_type = "2") {
  mySDI12.clearBuffer();
  String command = "";
  command += i;
  command += "C";
...
//----------------------------------------------------------------------bettery----------------------
bool getResultsBattery(char i, int resultsExpected) {
  uint8_t resultsReceived = 0;
  uint8_t cmd_number      = 0;
  while (resultsReceived < resultsExpected && cmd_number <= 9) {
    String command = "";
...
int startConcurrentMeasurementBattery(char i, String meas_type = "9") {
  mySDI12.clearBuffer();
  String command = "";
  command += i;
  command += "C";
...
void loop() {
  ...
  for (byte i = 0; i < 62; i++) {
    char addr = decToChar(i);
    if (isActive[i]) { startConcurrentMeasurementTemperature(addr); 
      ...
      if (isActive[i]) {
        if (millis() > millisReady[i]) {
          if (returnedResults[i] > 0) {
            ...
            getResultsTemperature(addr, returnedResults[i]);
 ...
 char addr = decToChar(1);
  startConcurrentMeasurementHumidity(addr); 
   // get all readings
   numReadingsRecorded = 0;
  while (numReadingsRecorded < numSensors) {
    for (byte i = 0; i < 62; i++) {
      char addr = decToChar(i);
      if (isActive[i]) {
        if (millis() > millisReady[i]) {
          if (returnedResults[i] > 0) {
            ...
            Serial.print(", ");
              ...
              getResultsHumidity(addr, returnedResults[i]);
...
delay(1000);  // wait ten seconds between measurement attempts.
 ...
 addr = decToChar(1);
startConcurrentMeasurementBattery(addr); 
   // get all readings
   numReadingsRecorded = 0;
  while (numReadingsRecorded < numSensors) {
    for (byte i = 0; i < 62; i++) {
  ...
  Serial.print("Battery:       ");
                 getResultsBattery(addr, returnedResults[i]);
            Serial.println();
          }
          numReadingsRecorded++;
        }
